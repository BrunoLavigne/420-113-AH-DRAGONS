...
public class MembreDAO extends DAO {
	...
	private static final String ADD_REQUEST = "INSERT INTO membre (nom, telephone, limitePret, nbPret) VALUES (?, ?, ?, 0)";
	...
	public void add(MembreDTO membreDTO) throws DAOException {
		...
		try (PreparedStatement addPreparedStatement = getConnection().prepareStatement(ADD_REQUEST)) {
			addPreparedStatement.setString(1, membreDTO.getNom());
			addPreparedStatement.setLong(2, membreDTO.getTelephone());
			addPreparedStatement.setInt(3, membreDTO.getLimitePret());
			addPreparedStatement.execute();
		} catch (SQLException sqlException) {
			throw new DAOException(Integer.toString(sqlException.getErrorCode()), sqlException);
		}
	}
	...
	public void update(MembreDTO membreDTO) throws DAOException {
		...
		try (PreparedStatement updatePreparedStatement = getConnection().prepareStatement(UPDATE_REQUEST)) {
			...
			updatePreparedStatement.setLong(2, membreDTO.getTelephone());
			updatePreparedStatement.setInt(3, membreDTO.getLimitePret());
			updatePreparedStatement.setInt(4, membreDTO.getNbPret());
			updatePreparedStatement.setInt(5, membreDTO.getIdMembre());
