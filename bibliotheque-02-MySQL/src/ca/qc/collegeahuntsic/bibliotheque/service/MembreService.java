...
public class MembreService extends Services {
	...
	public void inscrire(MembreDTO membreDTO) throws ServiceException {
		...
		try {
			...
			if (getMembreDAO().read(membreDTO.getIdMembre()) != null) {
				System.err.println("Un membre avec l'id " + membreDTO.getIdMembre() + " existe déjà.");
				return;
	...
	public void emprunter(MembreDTO membreDTO, LivreDTO livreDTO) throws ServiceException {
		...
		try {
			...
			if (getMembreDAO().read(membreDTO.getIdMembre()) == null) {
				System.err.println("Le membre avec l'ID " + membreDTO.getIdMembre() + " n'existe pas.");
				return;
			...
			if (membreDTO.getNbPret() >= membreDTO.getLimitePret()) {
				System.err.println("Le membre avec l'ID " + membreDTO.getIdMembre() + " a atteint sa limite de prêts.");
				return;
				/*
				 * throw new ServiceException( "Le membre avec l'ID " +
			...
			if (!getLivreDAO().checkLivreExist(livreDTO.getIdLivre())) {
				System.err.println("Le livre avec l'ID " + livreDTO.getIdLivre() + " n'existe pas.");
				return;
	...
	public void renouveler(MembreDTO membreDTO, LivreDTO livreDTO) throws ServiceException {
		...
		try {
			...
			if (unMembreDTO == null) {
				System.err.println("Le membre : " + membreDTO.getIdMembre() + " n'existe pas");
				return;
			...
			if (unLivreDTO == null) {
				System.err.println("Le livre : " + livreDTO.getIdLivre() + " n'existe pas");
				return;
			...
			if (emprunteur == null) {
				System.err.println("Le livre : " + unLivreDTO.getIdLivre()
			...
			if (unMembreDTO.getIdMembre() != emprunteur.getIdMembre()) {
				System.err.println("Le livre : " + unLivreDTO.getIdLivre() + " est déjà prêté au membre : "
	...
	public void desinscrire(MembreDTO membreDTO) throws ServiceException {
		...
		try {
			...
			if (membreDTO.getNbPret() > 0) {
				System.err
						.println("Le membre avec l'ID" + membreDTO.getIdMembre() + " a encore " + membreDTO.getNbPret()
			...
			if (listeReservations.size() > 0) {
				System.err.println("Le membre avec l'ID " + membreDTO.getIdMembre() + " a encore "
